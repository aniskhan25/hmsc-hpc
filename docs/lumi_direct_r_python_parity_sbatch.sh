#!/bin/bash -l
#SBATCH --job-name=direct-rpy-parity
...
RUN_NAME="${RUN_NAME:-direct_r_python_parity_${SLURM_JOB_ID:-manual}}"
...
CHAINS="${CHAINS:-2}"
VERBOSE="${VERBOSE:-100}"
RNG_SEED="${RNG_SEED:-20260718}"
FP="${FP:-64}"
RSCRIPT="${RSCRIPT:-Rscript}"
R_MODULE="${R_MODULE:-cray-R/4.4.0}"
R_LIBS_USER="${R_LIBS_USER:-${USER_WORK}/R/library/4.4}"
CONFIGS="${CONFIGS:-examples/projects/simulated_poisson_recovery/model.yaml examples/projects/simulated_spatial_validation/model_iid.yaml}"
...
for CONFIG in ${CONFIGS}; do
  ...
  mkdir -p "${CASE_ROOT}"
  echo "Running direct R/Python parity for ${CONFIG} -> ${CASE_ROOT}"

  "${PYTHON}" examples/run_direct_r_python_parity.py \
    --config "${CONFIG}" \
    --output "${CASE_ROOT}" \
    --samples "${SAMPLES}" \
    --transient "${TRANSIENT}" \
  ...
  "${RSCRIPT}" "${CASE_ROOT}/r_bridge/make_init.R"
  ...
  "${PYTHON}" examples/run_direct_r_python_parity.py \
    ...
    --posterior-gates diagnostic \
    --strict
done

printf 'wall_time_seconds=%s\n' "${SECONDS}" | tee "${RUN_ROOT}/wall_time.txt"
echo "Direct R/Python parity complete: ${RUN_ROOT}"

#!/bin/bash -l
#SBATCH --job-name=pyhmsc-newfeat-val
...
RUN_NAME="${RUN_NAME:-new_features_validation_${SLURM_JOB_ID:-manual}}"
RUN_ROOT="${USER_WORK}/hmsc-hpc-runs/${RUN_NAME}"
PROJECT_DIR="${REPO_DIR}/examples/projects/simulated_new_features_validation"
...
run_model "random_fixed" "${PROJECT_DIR}/random_slope/model_fixed.yaml"
...
"${PYTHON}" examples/analyze_new_features_validation.py \
  --project "${PROJECT_DIR}" \
  --random-fixed-posterior "${RUN_ROOT}/random_fixed/posterior.h5" \
...
echo
echo "Random fixed posterior: ${RUN_ROOT}/random_fixed/posterior.h5"

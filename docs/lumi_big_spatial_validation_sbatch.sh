#!/bin/bash -l
#SBATCH --job-name=pyhmsc-big-spatial
...
# End-to-end real-data spatial validation run using the compact big_spatial
# plant project. Submit from the hmsc-hpc repository root:
#
...
RUN_NAME="${RUN_NAME:-big_spatial_real_${SLURM_JOB_ID:-manual}}"
...
"${PYTHON}" examples/analyze_big_spatial_plants.py \
  ...
  --output "${RUN_ROOT}/big_spatial_validation_report.txt"
...
echo "Validation report: ${RUN_ROOT}/big_spatial_validation_report.txt"
